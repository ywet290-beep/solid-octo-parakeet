...
const typingUsers: { [roomId: string]: { [userId: string]: { username: string; timestamp: number } } } = {};
...
export const initializeSocket = (io: Server) => {
  ...
  io.on('connection', (socket: AuthenticatedSocket) => {
    ...
    // Edit message
    socket.on('edit-message', async (data: { messageId: string; newContent: string; roomId: string }) => {
      try {
    ...
    socket.on('answer-call', (data: { to: string; signalData: any }) => {
